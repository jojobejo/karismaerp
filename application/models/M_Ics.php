...
class M_Ics extends CI_Model
{
    ...
    public function list_barang_ics()
    {
        return $this->db->query("SELECT 
            ...
            SUM(FLOOR((i.qty - COALESCE(od.qty_out, 0)) / (mb.p * mb.l * mb.t))) AS saldo_akhir_box,
            SUM(MOD((i.qty - COALESCE(od.qty_out, 0)), (mb.p * mb.l * mb.t)))    AS saldo_akhir_pcs,
            CASE
                WHEN
                    SUM(FLOOR((i.qty - COALESCE(od.qty_out, 0)) / (mb.p * mb.l * mb.t))) =
                    SUM(COALESCE(FLOOR(op.qty_op / (mb.p * mb.l * mb.t)), 0))
                AND
                    SUM(MOD((i.qty - COALESCE(od.qty_out, 0)), (mb.p * mb.l * mb.t))) =
