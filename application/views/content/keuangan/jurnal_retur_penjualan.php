...
<style>
    ...
    .jurnal-list-page .retur-journal-search { width: min(360px, 100%); }
                ...
                <div class="container-fluid">
                    <div class="page-title-row">
                        <div class="page-title-left">
                            ...
                            <h1 class="page-title">Jurnal Retur Penjualan</h1>
                        </div>
                    </div>

                    <div class="list-panel">
                        <div class="panel-heading">
                            <span>Daftar Jurnal Retur Penjualan</span>
                            <small id="returJournalCount">0 data</small>
                        </div>
                        <div class="list-toolbar">
                            <div class="text-muted"><i class="fas fa-info-circle mr-1"></i> Klik baris untuk melihat detail jurnal debit/kredit.</div>
                            <input type="text" class="form-control retur-journal-search" id="returJournalSearch" placeholder="Cari referensi, SPR, pelanggan...">
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover table-striped zahir-table">
                                ...
                                <tbody id="returJournalRows">
                                    <tr><td colspan="6" class="text-center text-muted py-4">Memuat jurnal retur penjualan...</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                ...
                <div class="modal-content">
                    <div class="modal-body">
                        <div class="zahir-detail-head">
                            <span id="salesJournalRef">RJP</span>
                            <span id="salesJournalDate">-</span>
                            <span id="salesJournalTitle">Retur Penjualan</span>
                        </div>
                        <div class="table-responsive">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function formatMoney(num) {
        ...
        return 'Rp ' + p.toLocaleString('id-ID', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    }
    ...
    function renderRows(rows) {
        $('#returJournalCount').text((rows || []).length + ' data');
        if (!rows || !rows.length) {
            $('#returJournalRows').html('<tr><td colspan="6" class="text-center text-muted py-4">Data jurnal retur penjualan tidak ditemukan.</td></tr>');
            return;
        }
        ...
        $('#returJournalRows').html(html);
    }

    function loadList(searchValue) {
        if (!schemaReady) return;
        $.ajax({
            url: endpointBase + '/retur-list',
            type: 'POST',
            dataType: 'json',
    ...
    $('#returJournalSearch').on('input', debounce(function() {
        loadList($(this).val().trim());
    }, 300));

    $('#returJournalRows').on('click', 'tr', function() {
        var id = $(this).data('id');
        if (!id) return;
        loadDetail(id);
    });
    ...
    function loadDetail(id) {
        $.ajax({
            ...
            success: function(resp) {
                ...
                $('#salesJournalTitle').text(header.keterangan || 'Retur Penjualan');
