<!-- application/views/content/keuangan/kas_masuk_list.php -->
<style>
    :root {
        --zahir-blue: #127fad;
...
$(document).ready(function() {
    ...
    // Submit filter
    $('#formFilterKasMasuk').submit(function(e) {
        e.preventDefault();
        loadData();
    ...
    $('#btn-baru').click(function() {
        window.location.href = "<?= base_url('keuangan/kas_masuk/form') ?>";
    });

    // Row selection
    $(document).on('click', '#list-kas-masuk-body tr', function() {
        if ($(this).hasClass('empty-row')) return;

        $('#list-kas-masuk-body tr').removeClass('selected');
    ...
    $(document).on('dblclick', '#list-kas-masuk-body tr', function() {
        if ($(this).hasClass('empty-row')) return;
        let id = $(this).data('id');
        window.location.href = "<?= base_url('keuangan/kas_masuk/form/') ?>" + id;
    });

    // Detail button
    $('#btn-perincian').click(function() {
        if (selectedId) {
            window.location.href = "<?= base_url('keuangan/kas_masuk/form/') ?>" + selectedId;
        }
    });
    ...
    $('#btn-cetak').click(function() {
        if (selectedId && selectedStatus === 'POSTED') {
            window.open("<?= base_url('keuangan/kas_masuk/print_receipt/') ?>" + selectedId, '_blank');
        }
    });
    ...
    $('#btn-hapus').click(function() {
        ...
        if (confirm(confirmMsg)) {
            $.ajax({
                url: "<?= base_url('keuangan/kas_masuk/delete') ?>",
                type: "POST",
                data: { id_kas_masuk: selectedId },
                dataType: "json",
    ...
    function loadData() {
        ...
        $.ajax({
            url: "<?= base_url('keuangan/kas_masuk/get_data') ?>",
            type: "POST",
            data: {
                search: search,
                date_from: date_from,
                ...
                if (res.success) {
                    ...
                    if (res.data.length > 0) {
                        res.data.forEach(function(row) {
                            ...
                            html += `<tr data-id="${row.id_kas_masuk}" data-status="${row.status}" ${textStyle}>
                                <td>${row.tanggal_formatted}</td>
                                <td style="font-weight: bold; color: var(--zahir-blue);">${row.no_referensi}</td>
                                <td>${escapeHtml(row.diterima_dari)}</td>
                        ...
                        html = '<tr class="empty-row"><td colspan="6" class="text-center text-muted">Tidak ada data Kas Masuk.</td></tr>';
                    }
                    $('#list-kas-masuk-body').html(html);
                    clearSelection();
                }
            }
        });
