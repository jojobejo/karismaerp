<!-- application/views/content/keuangan/kas_keluar_form.php -->
<style>
    :root {
        --zahir-blue: #127fad;
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        <div class="form-container">
            <form id="form-kas-keluar">
                <input type="hidden" name="id_kas_keluar" id="id_kas_keluar" value="<?= $header ? $header['id_kas_keluar'] : '' ?>">
                
                <div class="zahir-card">
                    <div class="card-header-zahir">
                        <h3><i class="fas fa-money-bill-wave"></i> Kas Keluar</h3>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group row form-group-zahir">
                                    <label class="col-sm-3 col-form-label">Penerima : <span class="text-danger">*</span></label>
                                    <div class="col-sm-9">
                                        <div class="input-lookup-wrap">
                                            <input type="text" class="form-control form-control-zahir w-100" name="penerima" id="penerima" value="<?= $header ? htmlspecialchars($header['penerima']) : '' ?>" placeholder="Ketik atau cari penerima..." required>
                                            <button type="button" class="btn-lookup-trigger" id="btn-lookup-penerima"><i class="fas fa-search"></i></button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group row form-group-zahir">
                                    <label class="col-sm-3 col-form-label">Cek No. : <span class="text-danger">*</span></label>
                                    <div class="col-sm-9">
                                    ...
                                    <table class="table-grid" id="table-alokasi-dana">
                                        <thead>
                                            <tr>
                                                ...
                                                <th style="width: 25%; text-align: right;">Nilai (Dr)</th>
                                            </tr>
                                        </thead>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.detail-kode, .detail-nama', function() {
        if ($('#form-kas-keluar').data('disabled') === true) return;
    ...
    function loadAccountsLookup(searchQuery) {
        $.ajax({
            url: "<?= base_url('keuangan/kas_keluar/accounts_lookup') ?>",
            type: "GET",
    ...
    function calculateTotal() {
        ...
        $.ajax({
            url: "<?= base_url('keuangan/kas_keluar/terbilang_ajax') ?>",
            type: "POST",
            data: { amount: total },
            dataType: "json",
    ...
    $('#form-kas-keluar').submit(function(e) {
        ...
        $.ajax({
            url: "<?= base_url('keuangan/kas_keluar/save') ?>",
            type: "POST",
            data: formData,
            dataType: "json",
            success: function(res) {
    ...
    // Lookup Penerima trigger
    $('#btn-lookup-penerima').click(function() {
        alert('Ketik nama penerima langsung pada kolom input Penerima.');
    });
});
</script>
</body>
