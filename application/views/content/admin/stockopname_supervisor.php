    ...
    <div class="content-wrapper supervisor-opname-page">
        <section class="content">
            <div class="container-fluid py-3 pb-4">
                <style>
                    .supervisor-opname-page{background:#eef3f8}.sup-shell{max-width:520px;margin:0 auto}.sup-panel{background:#fff;border:1px solid #dce5ee;border-radius:8px;box-shadow:0 8px 22px rgba(15,23,42,.07)}.sup-panel-head{padding:14px 16px;border-bottom:1px solid #e6edf4;display:flex;align-items:center;justify-content:space-between;gap:10px}.sup-title{font-size:16px;font-weight:800;color:#172033;margin:0}.sup-muted{color:#64748b;font-size:12px}
                    .sup-user{display:flex;align-items:center;gap:12px;padding:16px}.sup-avatar{width:44px;height:44px;flex:0 0 44px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:#e0edff;color:#1d4ed8;font-size:18px}.sup-user-name{font-size:15px;font-weight:800;color:#172033}.sup-user-meta{display:flex;flex-wrap:wrap;gap:6px 12px;margin-top:4px;font-size:12px;color:#64748b}.sup-user-meta span{display:inline-flex;align-items:center;gap:5px}.sup-actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:0 16px 16px}.sup-actions .btn{font-weight:800}
                    .sup-list{display:grid;gap:10px}.sup-request{background:#f8fafc;border:1px solid #dbe4ef;border-radius:8px;padding:12px}.sup-request-name{font-size:14px;font-weight:800;color:#111827;line-height:1.35}.sup-request-meta{font-size:12px;color:#64748b;margin-top:5px;display:flex;flex-wrap:wrap;gap:5px 12px}.sup-region{display:inline-flex;align-items:center;gap:5px;color:#1d4ed8;font-weight:800}.sup-status{display:inline-block;padding:3px 8px;border-radius:999px;background:#e2e8f0;color:#334155;font-size:10px;font-weight:900}.sup-empty{text-align:center;color:#64748b;padding:28px 12px}
                    .sup-qty-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:10px}.sup-qty{background:#fff;border:1px solid #e1e8f0;border-radius:8px;padding:8px;text-align:center;min-width:0}.sup-qty small{display:block;color:#64748b;font-size:10px;font-weight:800;text-transform:uppercase}.sup-qty strong{display:block;color:#172033;font-size:16px;font-weight:900}
                    .sup-filter{padding:12px 16px;border-bottom:1px solid #e6edf4}.sup-filter label{font-size:11px;font-weight:800;color:#475569;text-transform:uppercase}.sup-request-action{display:flex;justify-content:flex-end;margin-top:10px}.sup-request-action .btn{font-weight:800}.sup-pagination{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:14px}.sup-pagination .btn{font-weight:800}.sup-chart-grid{display:grid;grid-template-columns:1fr;gap:12px;padding:16px}.sup-chart-card{border:1px solid #e1e7ef;border-radius:8px;padding:12px}.sup-chart-title{font-size:12px;font-weight:800;color:#334155;text-transform:uppercase}.sup-chart-meta{font-size:11px;color:#64748b}.sup-chart-wrap{height:220px}
                </style>
                ...
                <div class="sup-shell">
                    ...
                    <div class="sup-panel mb-3">
                        <div class="sup-panel-head"><h1 class="sup-title">Stockopname Result</h1></div>
                        <div class="sup-chart-grid">
                            <div class="sup-chart-card"><div class="sup-chart-title">All Barang</div><p class="sup-chart-meta" id="supAllMeta">0% match; Tim 1 atau Tim 2</p><div class="sup-chart-wrap"><canvas id="supAllChart"></canvas></div></div>
                            <div class="sup-chart-card"><div class="sup-chart-title">By Expired Date + LOT</div><p class="sup-chart-meta" id="supExpiredMeta">0% match; Tim 1 atau Tim 2</p><div class="sup-chart-wrap"><canvas id="supExpiredChart"></canvas></div></div>
                        </div>
                    </div>
...
$(function () {
    ...
    function renderResult(canvasId, metaId, data, colors) {
        data = data || {}; var match = parseInt(data.match || 0, 10), notMatch = parseInt(data.not_match || 0, 10);
        $('#' + metaId).text(Number(data.persen_match || 0).toLocaleString('id-ID') + '% match; Tim 1 atau Tim 2');
        var canvas = document.getElementById(canvasId);
        if (canvas && typeof Chart !== 'undefined') new Chart(canvas, {type:'pie',data:{labels:['Match','Not Match'],datasets:[{data:[match,notMatch],backgroundColor:colors,borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}}}});
    }
    ...
    $('.btn-affirm').on('click', function () {
        var button = $(this), id = parseInt(button.data('id'), 10);
        if (!id) return;
        var submit = function () {
            button.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Memproses');
            $.post('<?= base_url('supervisi-opname/afirmasi') ?>', {id:id}, null, 'json')
                .done(function (res) {
        ...
        Swal.fire({title:'Afirmasi request?',text:'Data akan disimpan ke hasil opname dan request ditandai selesai.',icon:'question',showCancelButton:true,confirmButtonText:'Ya, afirmasi',cancelButtonText:'Batal'})
