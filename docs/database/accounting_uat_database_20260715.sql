...
-- Jalankan dari database aplikasi, bukan information_schema.
-- Di local XAMPP biasanya: USE `kiucoid_karismaerp_local`;
...
FROM (
  ...
  SELECT 'PURCHASE_INVOICE','GRNI','DEBIT' UNION ALL
  SELECT 'PURCHASE_INVOICE','VAT_INPUT','DEBIT' UNION ALL
  SELECT 'PURCHASE_INVOICE','ACCOUNT_PAYABLE','KREDIT' UNION ALL
